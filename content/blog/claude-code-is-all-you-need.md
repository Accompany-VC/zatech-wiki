...
summary = "How Gareth Dwyer uses Claude Code for work, fun, and as a text editor."
author = "Gareth Dwyer"
author_social = "https://dwyer.co.za/"
original_url = "https://dwyer.co.za/static/claude-code-is-all-you-need.html"
math = false
body_class = "claude-guide"
+++

<div class="claude-hero">
  <h1 class="claude-hero__title">Claude Code<br>Is All You<br>Need</h1>
  <p class="claude-hero__subtitle">How I use Claude Code for work, fun, and as a text editor</p>
</div>

![Claude Code](https://dwyer.co.za/static/img/claude-code-header.png "Claude Code hero banner")
...
![Micromonitor autonomous startup demo](https://dwyer.co.za/static/img/micromonitor.png "Autonomous startup demo")

![SmartSplit expense splitting app](https://dwyer.co.za/static/img/smartsplit-php-demo.png "SmartSplit demo")

![Poster maker interface](https://dwyer.co.za/static/img/poster-maker.png "Poster maker demo")

![Hacker News comment ranker plugin](https://dwyer.co.za/static/img/hn-plugin-demo.png "HN plugin demo")

![AWTP Trello alternative](https://dwyer.co.za/static/img/awtp.png "AWTP project")

![Bank statements organization tool](https://dwyer.co.za/static/img/bank-statements.png "Bank renaming tool")
...
## Let's vibe code some CRUD {#vibe-code}

<p class="claude-section-intro">Let's try some vibe coding.</p>

<div class="claude-aside"><strong>Aside:</strong> The definition of vibe coding is still in flux, but to me it means creating software without looking at or editing code. You don't really care what languages or frameworks are used under the hood and develop the code only by chatting with a model.</div>
...
I cheated a bit because the prompt I used to one-shot this is based on earlier attempts where the model did things that I didn't want, but the app shown below and at [smartsplit.verysmall.site](https://smartsplit.verysmall.site/) is the output of `claude -p "Read the SPEC.md file and implement it"`. My SPEC.md file is about 500 words (shown in full a bit later).

![Working Smartsplit PHP demo](https://dwyer.co.za/static/img/smartsplit-php-demo.png "Working SmartSplit PHP demo")
...
![Failed SmartSplit JavaScript version](https://dwyer.co.za/static/img/smartsplit-js-fail.png "Overcomplicated JavaScript attempt")

## A tale of two vibe-coded codebases {#tale-two-codebases}

Let's take a look at these two projects and the prompt that created them. The working version is a 900-line index.php file that contains the entire app. The broken version is a NodeJS project split into a client and a server. It's not much longer in terms of lines of code—about 1000 lines of non-dependency code split up over 15 files. But after you run `npm i` on the broken version, it pulls in 500MB (!!) of dependencies.

![SmartSplit project structure comparison](https://dwyer.co.za/static/img/smartsplit-tree.png "Comparing SmartSplit PHP and JS versions")

Here's the full SPEC.md prompt I gave Claude Code. It's nearly the same in both cases, except for the PHP version where I tell it to keep things simple, stay away from frameworks, and just write raw SQL. In the broken version, I let it do whatever it wants.
...
* Each SmartSplit gets a unique 8-digit alphanumeric code that makes it easy to share URLs
* Any user can create a new smartsplit, or join an existing one if they're logged in with an email address that was invited
* Even if that user doesn't yet have an account on SmartSplit, they'll have access to any SmartSplits they're added to after signing up (This bit is important). If the invitee has added their email address, they should have access automatically.
...
* Passwords are hashed but no extra security like length or weak passwords or confirmed passwords is applied
...
Yes, it's a toy example and some people will say that the JavaScript one scales better or something. I'm not here to fight. I hate PHP too, but I'm using it more often for fully vibe-coded apps because LLMs are very good at it. Frameworks and abstractions are for humans at the end of the day, not robots, and often they get in the way of vibe coding instead of being helpful.

![Levels.io tweet about simple tech stacks](https://dwyer.co.za/static/img/levels-simple.png "@levelsio on keeping tech stacks simple")

[@levelsio on keeping things simple](https://x.com/levelsio/status/1945125163793609032)

## Building an autonomous startup {#autonomous-startup}
...
## Giving a root VPS to a robot and saying 'go fetch' {#root-vps}
...
![Micromonitor startup](https://dwyer.co.za/static/img/micromonitor.png "Micromonitor prototype built by Claude Code")
...
![Claude monitoring](https://dwyer.co.za/static/img/monitor-claude.png "Watcher script keeping Claude running")

<div class="claude-aside"><strong>Aside:</strong> I hit a small snag where Anthropic decides that running Claude as root with --dangerously-skip-permissions / yolo-mode is not allowed. You can get past this dumb nanny-state stuff by running:</div>
...
<div class="claude-aside"><strong>Aside:</strong> Most people I know would criticize this in the same way that AI has always been criticized. "It's not real bro, it's just pattern matching. It's seen stuff like that before. It's not even working properly. An intern could do that with a bit of time."</div>
...
## Hitting a snag: the model builders are also the police now {#hitting-snag}
...
![Anthropic usage policy](https://dwyer.co.za/static/img/aup-human.png "Anthropic usage policy warning")
...
![Writing in Claude Code](https://dwyer.co.za/static/img/editor.png "Writing inside the Claude Code terminal UI")

## Migrating a real production project {#production-migration}
...
![Generated README](https://dwyer.co.za/static/img/readme.png "Claude-generated README")
...
<div class="claude-aside"><strong>Aside:</strong> It _did_ drop the soon-to-be production database once when I really wasn't expecting it to and it was not the appropriate thing to do based on what it was trying to do. Remember before when I said you needed faith? That also means expecting stuff like this to happen and forgiving, forgetting and moving on instead of trying to hype up the internet and the media about ['AI GOING ROGUE'](https://www.reddit.com/r/OpenAI/comments/1m4lqvh/replit_ai_went_rogue_deleted_a_companys_entire/).</div>
...
## You can just build things {#build-things}
...
### Building a Hacker News comment ranker plugin

I've often been annoyed by comments on Hacker News that are not at all about the article they're commenting on. "Bitcoin adopts a new FlibbityGippity Protocol and can now handle 2.3 transactions per day" and someone will comment that all crypto projects are scams or something. Note that I don't care about the quality of the comment, or whether or not I agree with it, but I wanted a visual way to skip over the 'noise' comments that aren't actually about the article at all.

I tried to build this before but got distracted by more important stuff, so I figured I'd start over with Claude Code.

![Hacker News comment ranking plugin demo](https://dwyer.co.za/static/img/hn-plugin-demo.png "Hacker News comment ranker badges")
...
![Hacker News plugin settings page](https://dwyer.co.za/static/img/hn-plugin-settings.png "Plugin settings for comment ranking")
...
![Poster Maker interface](https://dwyer.co.za/static/img/poster-maker.png "Poster Maker interface")

<div class="claude-aside"><strong>Aside:</strong> Another small snag, it seems like OpenAI blocks Anthropic crawlers so Claude couldn't go read the OpenAI API docs and figure out how to image gen. I had to save the file locally and tell it to reference that.</div>
...
![Bank statements renaming tool](https://dwyer.co.za/static/img/bank-statements.png "Renamed and categorised bank statements")
...
![This article being written](https://dwyer.co.za/static/img/this-article.png "Drafting this article in Claude Code")
